...
cmake -S "$SMINA" -B "$SMINA/build" \
  ...
  -DBoost_INCLUDE_DIR="$PREFIX/include" -DBoost_NO_BOOST_CMAKE=OFF \
  -DCMAKE_CXX_FLAGS="-std=c++17 -I$PREFIX/include -I$PREFIX/include/eigen3 -DBOOST_TIMER_ENABLE_DEPRECATED -DBOOST_ALLOW_DEPRECATED_HEADERS -DEIGEN_MAX_ALIGN_BYTES=0 -DEIGEN_MAX_STATIC_ALIGN_BYTES=0 -DEIGEN_NO_DEBUG"
